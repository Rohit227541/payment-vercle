...
export default function AdminMerchants() {
  ...
  const verifyMerchantKyc = async (merchantId: string, status: 'APPROVED' | 'REJECTED') => {
    try {
      const res = await apiFetch(`/admin/merchant/verify-merchant-kyc/${merchantId}/kyc`, {
        method: 'PATCH',
        body: JSON.stringify({ kycStatus: status, verificationNotes }),
      }, true);
      if (res.success) {
