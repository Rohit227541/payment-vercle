...
export default function KeyManagement() {
  ...
  const fetchAllMerchantCredentials = async () => {
    ...
    try {
      ...
      if (allMerchants.length > 0) {
        // Fetch real API credentials for each merchant in parallel
        const credentialsPromises = allMerchants.map(async (m: any) => {
          const mId = m.merchantId || m.merchant_id;
          
          let realApiKey = 'NOT GENERATED';
          let realSecretKey = 'NOT GENERATED';
          let realWebhookUrl = 'N/A';
          let realStatus = 'INACTIVE';
          let realCredId = `cred-${mId}`;

          if (mId) {
            try {
              const apiRes = await apiFetch(`/admin/api/${mId}`, {}, true);
              if (apiRes && apiRes.success && apiRes.data && apiRes.data.credentials && apiRes.data.credentials.length > 0) {
                // The backend returns an array of credentials inside data.credentials
                // For simplicity, we take the first active one, or just the first one.
                const cred = apiRes.data.credentials.find((c: any) => c.status === 'ACTIVE') || apiRes.data.credentials[0];
                
                realApiKey = cred.publicKey || cred.public_key || 'NOT GENERATED';
                // Secret keys shouldn't be returned by GET requests for security, so we show dots if a key exists
                realSecretKey = realApiKey !== 'NOT GENERATED' ? '••••••••••••••••' : 'NOT GENERATED';
                realStatus = cred.status || 'ACTIVE';
                realCredId = cred.credentialId || cred.credential_id || `cred-${mId}`;
              }
            } catch (err) {
              // Ignore individual credential fetch errors, they might just not exist yet
            }
          }
          ...
          return {
            ...
            status: realStatus,
            createdAt: m.createdDate || m.createdAt || m.created_at || new Date().toISOString()
          };
        });

        const credsList: AdminMerchantApiItem[] = await Promise.all(credentialsPromises);
  ...
  const currentCredentials = filteredCredentials.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-ink-200/60 dark:border-ink-800/60 bg-white dark:bg-ink-900/60 backdrop-blur-xl shadow-sm overflow-hidden">
        ...
        {totalPages > 1 && !loading && filteredCredentials.length > 0 && (
